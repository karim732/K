...
const SmMdLeftNav = () => {
  ...
  return <div style={{width: "100%", height: "100%", backgroundColor: "rgba(0, 0, 0, 0.4)", position:"absolute"}} onClick={()=> setIsSmMdSideNavActive(false)}>
    <div style={{width: "75%", height: "100%", backgroundColor: "white",display: "flex", flexDirection: "column"}}>
      <div style={{padding: "15px"}}>
        <div className="d-flex justify-content-between">
          <img src={profile} alt="profile pic" width="50px" />
          <div>
            <FontAwesomeIcon icon={faPlus} style={{fontSize:"20px", border: "1px solid gray", borderRadius: "50%",padding: "8px", color: "#000000a8"}}/>
          </div>
        </div>
        <div className="d-flex flex-column">
          <span style={{fontSize: "25px", fontWeight: "600"}}>Karimshaik73 </span>
          <span style={{fontSize: "20px"}}>@karimshaik73</span>
        </div>
      </div>
      <div >
        {SmMdListItems.map( item => <ListItem key={item.key} item={item}/>)}
      </div>
  ...
  return (
    ...
    >
      <ul>
        ...
        <li>
        <PrimaryButton border={"pill"} title={isMediumScreen ? <FontAwesomeIcon icon={faFeatherPointed} /> :"Post"} width="100%"/>
        </li>
      </ul>
      <ul className={classes["profile-list"]}>
        <li>
          <img src={profile} alt="profile pic" width="40px" />
          { isMediumScreen || <><span className="flex-grow-1 d-flex flex-column">
